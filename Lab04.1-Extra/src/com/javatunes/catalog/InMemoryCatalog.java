...
public class InMemoryCatalog implements Catalog {
    ...
    public MusicItem cheapestItemByGenre(MusicCategory category) {
        ArrayList<MusicItem> result = (ArrayList<MusicItem>) findByCategory(category);
        result.sort(Comparator.comparingDouble(MusicItem::getPrice));
        return result.get(0);
    }
    ...
    public double averagePriceByGenre(MusicCategory category) {
        double sum = 0.0;
        Collection<MusicItem> result = findByCategory(category);
    ...
    public Collection<MusicItem> cheapSongsFromThe80s(double minPrice) {
        ...
        for (MusicItem item : catalogData) {
            if (item.getReleaseDate().toLocalDate().getYear() > 1979
    ...
    public Map<MusicCategory, Collection<MusicItem>> getCategoryMap() {
        ...
        for (MusicCategory c : MusicCategory.values()) {
            result.put(c, findByCategory(c));
        }
        return result;
    }
